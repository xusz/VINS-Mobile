...
DrawResult::DrawResult(float _pitch, float _roll, float _yaw,
...
bool checkBorder(const cv::Point2f &pt)
{
    ...
    return BORDER_SIZE <= img_x && img_x < HEIGHT - BORDER_SIZE
        && BORDER_SIZE <= img_y && img_y < WIDTH - BORDER_SIZE;
}
...
float check_scale(const cv::Point2f &pt)
{
    ...
    if (BORDER_SIZE <= img_x && img_x < WIDTH - 10 - BORDER_SIZE
     && BORDER_SIZE <= img_y && img_y < HEIGHT - 10 - BORDER_SIZE)
    {
        scale_factor = 1.0;
    }
    else
...
vector<Vector3f> DrawResult::calculate_camera_pose(Vector3f camera_center,
                                                   Matrix3f Rc, float length)
{
...
Vector3f DrawResult::findGround(vector<Vector3f> &point_cloud,
...
void DrawResult::drawGround(cv::Mat &result,
...
void DrawResult::drawBox(cv::Mat &result,
                         Vector3f corner_0,
                         Vector3f corner_x,
                         Vector3f corner_y,
                         Vector3f corner_z,
...
void DrawResult::drawAR(cv::Mat &result,
                        vector<Vector3f> &point_cloud,
                        Vector3f P_latest,
                        Matrix3f R_latest)
{
...
//    if (point_inlier.size() > 28)
    ...
    for (unsigned int i = 0; i < Grounds.size(); i++)
    {
        ...
        {
            drawBox(result, Grounds[i].ori,
                    Grounds[i].cox, Grounds[i].coy, Grounds[i].coz,
    ...
    if (locationX != locationX_p || locationY != locationY_p)
    {
        ...
        if (dis_min < 120 && Grounds.size() > 0 && Grounds[dis_min_idx].moveflag && finger_state == 1)
        {
            ...
            float rotation_angle = atan2(sqrt((cy-yy)*(cy-yy) + (xx-cx)*(xx-cx)), FOCUS_LENGTH_X);
    ...
    ///////////////////////////// rotation response
    {
        ...
        if (X0AR != X0_p || Y0AR != Y0_p)
        {
            if (dis_min < 120 && Grounds.size()>0 )
            {
                ...
                Vector3f TT = Grounds[dis_min_idx].center;
    ...
    // add new box
    if (tapFlag && point_inlier.size() > 20)
    {
...
void DrawResult::Reprojection(cv::Mat &result,
                              vector<Vector3f> &point_cloud,
                              const Matrix3f *R_window,
                              const Vector3f *T_window,
                              bool box_in_trajectorty)
{
    float depth_marker;
...
//        float scale_factor;
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        vector<Vector3f> camera_coner_w = calculate_camera_pose(T_window[i],
                                                                R_window[i],
                                                                length);
        ...
        CvScalar camera_color = cvScalar(0, 0, 255);  // RGB
        cv::line(result, camera_coner[0], camera_coner[1], camera_color, 1, 8, 0);
