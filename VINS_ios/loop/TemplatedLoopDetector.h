...
protected:
     ...
     * @return island score
     */
    double calculateIslandScore(const QueryResults &q,
    ...
    void reduceInputToOutput(vector<cv::Point2f> v_in,
                             vector<cv::Point2f> &v_out,
                             vector<uchar> status);
                          ...
                          std::vector<cv::Point2f> &old_pts)
{
    ...
    {
        m_database->getVocabulary()->transform(descriptors,
                                               bowvec,
                                               featvec,
                                               m_params.di_levels);
    }
    ...
    else if (!q.empty())
    {
        ...
        for (unsigned int idx = 1; dit != q.end(); ++dit, ++idx)
        {
            ...
            else
            {
                ...
                {
                    islands.push_back(tIsland(first_island_entry,
                                              last_island_entry,
                                            ...
                                            EntryId entry_id)
{
    ...
    if (m_window.nentries == 0
        || int(entry_id - m_window.last_query_id) > m_params.max_distance_between_queries)
    {
        m_window.nentries = 1;
    }
    else
...
// --------------------------------------------------------------------------
template<class TDescriptor, class F>
void TemplatedLoopDetector<TDescriptor, F>::reduceInputToOutput(vector<cv::Point2f> v_in,
                                                                vector<cv::Point2f> &v_out,
                                                                vector<uchar> status)
{
    v_out.clear();
                                                              ...
                                                              vector<cv::Point2f> &old_output_pts)
{
    ...
    {
        vector<uchar> status;
        cv::findFundamentalMat(old_input_pts,
                               cur_input_pts,
                               cv::FM_RANSAC,
                               1.0,
                               0.99,
                                        ...
                                        std::vector<cv::Point2f> &old_pts)
{
    ...
    if ((int)i_old.size() >= m_params.min_Fpoints)
    {
        ...
        bool find_fundamental_suss = checkFoundamental(cur_points,
                                                       old_points,
                                                       cur_pts,
                                                       old_pts);
                                                            ...
                                                            double threshold) const
{
    ...
    QueryResults::iterator qit = lower_bound(q.begin(), q.end(), aux, Result::geq);
