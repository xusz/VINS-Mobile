...
void KeyFrameDatabase::resample(vector<int> &erase_index)
{
    ...
    for (; it != keyFrameList.end(); )
    {
        ...
        if (it == keyFrameList.begin() || dis > min_dis
            || (*it)->has_loop || (*it)->is_looped)
        {
            dis = 0;
            last_P = tmp_t;
            it++;
...
{
    list<KeyFrame*>::iterator it = find(keyFrameList.begin(),
                                        keyFrameList.end(),
                                        pKF);
...
/**
 * pose_graph线程，图优化
 * a. FourDOFError结构体 遍历所有关键帧，添加该关键帧与之前五帧的残差
 * b. FourDOFWeightError结构体 如果是闭环关键帧，添加闭环残差
 * c. ceres进行图优化
 * d. 将优化的结果用于更新所有关键帧位置
 * e. 遍历所有关键帧，利用当前优化和优化前的Yaw轴的值，进行所有关键帧的Yaw矫正
 */
void KeyFrameDatabase::optimize4DoFLoopPoseGraph(int cur_index,
                                                 ...
                                                 Eigen::Matrix3d &loop_correct_r)
{
    ...
    for (; it != keyFrameList.end(); it++)
    {
        ...
        if ((*it)->global_index == earliest_loop_index
            ...
            || (*it)->has_loop
            || (*it)->is_looped
            || keyFrameList.size() < max_frame_num)
        {
            dis = 0;
            last_P = tmp_t;
    ...
    for (it = keyFrameList.begin(); it != keyFrameList.end(); it++)
    {
        ...
        if ((*it)->has_loop)
        {
            ...
            Vector3d relative_t((*it)->loop_info(0),
                                (*it)->loop_info(1),
                                (*it)->loop_info(2));
    ...
    for (it = keyFrameList.begin(); it != keyFrameList.end(); it++)
    {
        ...
        Quaterniond tmp_q;
        tmp_q = Utility::ypr2R(Vector3d(euler_array[i][0],
                                        euler_array[i][1],
                                        euler_array[i][2]));
        Vector3d tmp_t = Vector3d(t_array[i][0],
                                  t_array[i][1],
                                  t_array[i][2]);
        ...
        if (need_resample[i])
        {
            (*it)-> updatePose(r_drift_it * tmp_t + t_drift_it,
                               r_drift_it * tmp_r);
        }
        else
        {
